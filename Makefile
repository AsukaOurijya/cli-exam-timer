...
test: $(TARGET)
	...
	output="$$(printf '\n\n1\nx\n' | ./$(TARGET) 2>&1 || true)"; printf '%s' "$$output" | grep -q 'Would you like to add a title? (y/n)?' && printf '%s' "$$output" | grep -q 'Please enter y or n.'
	output="$$(printf '\n\n1\ny\nExam\nx\n' | ./$(TARGET) 2>&1 || true)"; printf '%s' "$$output" | grep -q 'Title:' && printf '%s' "$$output" | grep -q 'Would you like to add additional notes? (y/n)?'
	output="$$(printf '\n\n1\nn\ny\n' | ./$(TARGET) 2>&1 || true)"; printf '%s' "$$output" | grep -q 'Note \[Click enter to new line, click enter twice to proceed to timer\]:'
